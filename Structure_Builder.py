#!/usr/bin/env python3
"""
OPTIMIZED Structure Builder - Smaller supercells for faster simulations
Key optimization: Reduced from 8×8×8 to 6×6×6 (reduces atoms by ~60%)
...
class OptimizedAlloyStructureBuilder:
    def __init__(self, composition, lattice_params):
        """
        composition: dict like {'Al': 0.33, 'Fe': 0.33, 'Ni': 0.34}
    ...
    def write_lammps_data(self, positions, atom_types, box, filename, structure_type):
        ...
        with open(filename, 'w') as f:
            ...
            f.write(f"# Generated for Assignment 2 - MM309 (Optimized)\n\n")
            ...
            f.write("Masses\n\n")
            masses = {'Al': 26.9815, 'Fe': 55.845, 'Ni': 58.6934}
            for idx, el in enumerate(self.elements, 1):
...
def generate_compositions():
    """
    Generate compositions across ternary diagram
    Total: 21 compositions (same as before)
    """
    compositions = []

    # Pure elements (vertices) - 3
    compositions.append({'Al': 1.0, 'Fe': 0.0, 'Ni': 0.0})
    compositions.append({'Al': 0.0, 'Fe': 1.0, 'Ni': 0.0})
    compositions.append({'Al': 0.0, 'Fe': 0.0, 'Ni': 1.0})

    # Binary edges - 9
    for frac in [0.25, 0.50, 0.75]:
        compositions.append({'Al': frac, 'Fe': 1-frac, 'Ni': 0.0})
        compositions.append({'Al': frac, 'Fe': 0.0, 'Ni': 1-frac})
        compositions.append({'Al': 0.0, 'Fe': frac, 'Ni': 1-frac})

    # Interior ternary - 9
    compositions.append({'Al': 0.33, 'Fe': 0.34, 'Ni': 0.33})
    compositions.append({'Al': 0.34, 'Fe': 0.33, 'Ni': 0.33})
    compositions.append({'Al': 0.50, 'Fe': 0.25, 'Ni': 0.25})
    compositions.append({'Al': 0.25, 'Fe': 0.50, 'Ni': 0.25})
    compositions.append({'Al': 0.25, 'Fe': 0.25, 'Ni': 0.50})
    compositions.append({'Al': 0.40, 'Fe': 0.40, 'Ni': 0.20})
    compositions.append({'Al': 0.40, 'Fe': 0.20, 'Ni': 0.40})
    compositions.append({'Al': 0.20, 'Fe': 0.40, 'Ni': 0.40})
    compositions.append({'Al': 0.60, 'Fe': 0.20, 'Ni': 0.20})
...
def main():
    ...
    print("=" * 70)
    print("OPTIMIZED Al-Fe-Ni Ternary Alloy Structure Generator")
    ...
    for idx, comp in enumerate(compositions, 1):
        ...
        print(f"  Al: {comp['Al']:.2f}, Fe: {comp['Fe']:.2f}, Ni: {comp['Ni']:.2f}")

        comp_name = f"Comp{idx:02d}_Al{int(comp['Al']*100):02d}_Fe{int(comp['Fe']*100):02d}_Ni{int(comp['Ni']*100):02d}"
